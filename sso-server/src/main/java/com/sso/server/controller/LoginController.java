...
public class LoginController {
	...
	public String login(String serverUrl, String name, String password, Model model,
		...
		if (userInfo == null) {
			model.addAttribute(SsoConstant.LOGIN_RESPONSE, ExecuteResult.ok(serverUrl, SsoConstant.LOGIN_ERROR));
		...
		redisUtil.set(token, JSONObject.toJSONString(userInfo), GlobalConstant.SSO_TOKEN_TIME_OUT);
		return "redirect:" + backUrl(serverUrl, token);
	}

	/**
	 * 直接登录（应用端无token，请求sso获取）
	 * http://127.0.0.1:9090/sso/login?serverUrl=http://127.0.0.1:9091/one/auth1
	 */
	@GetMapping("login")
	public String login(String serverUrl, Model model, HttpServletRequest request) throws UnsupportedEncodingException {
		String token = CookieUtils.getCookie(request, SsoConstant.COOKIE_TOKEN_NAME);
		if (StringUtils.isBlank(token)) {
			model.addAttribute(SsoConstant.LOGIN_RESPONSE, ExecuteResult.ok(serverUrl, SsoConstant.LOGIN_UN));
			return SsoConstant.LOGIN_PAGE;
		}
	 ...
	 * http://127.0.0.1:9090/sso/logout?serverUrl=http://127.0.0.1:9091/one/index
	 */
	@GetMapping("logout")
